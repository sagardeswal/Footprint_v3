<?php
    ...
    $statement = mysqli_prepare($con,"SELECT usr_ID, usr_fullname, usr_email, usr_mobile, can_add_place FROM fp_user_details WHERE usr_email= ? AND usr_passwordhashcode= ? AND active='1'") or die(mysql_error()); 
    mysqli_stmt_bind_param($statement, "ss", $email, $passwordhashcode);
	mysqli_stmt_execute($statement);
	mysqli_stmt_store_result($statement);
	mysqli_stmt_bind_result($statement, $usr_ID, $usr_fullname, 
		$usr_email, $usr_mobile, $can_add_place);
    $userdetails = array();
	while(mysqli_stmt_fetch($statement)){
		$userdetails[usr_ID] = $usr_ID;
		$userdetails[usr_fullname] = $usr_fullname;
		$userdetails[usr_email] = $usr_email;
		$userdetails[usr_mobile] = $usr_mobile;
		$userdetails[can_add_place] = $can_add_place;
	}

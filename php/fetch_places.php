<?php 
    ...
    $statement = mysqli_prepare($con,"SELECT p.pl_ID, p.pl_title, p.pl_description, p.pl_loc_id, p.pl_category, p.pl_admin_id , l.loc_longitude, l.loc_latitude FROM fp_place_details p, pl_location_details l WHERE p.active='1' AND p.pl_loc_id = l.loc_id") or die(mysql_error()); 
	mysqli_stmt_execute($statement);
	mysqli_stmt_store_result($statement);
	mysqli_stmt_bind_result($statement,$pl_ID, $pl_title, $pl_description, $pl_loc_id, $pl_category, $pl_admin_id, $pl_loc_long, $pl_loc_lat);
    $placedetails = array();
    $i=0;
	while(mysqli_stmt_fetch($statement)){
		...
		$placedetails[$i][pl_admin_id] = $pl_admin_id;
